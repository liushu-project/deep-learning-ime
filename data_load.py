    ...
    with open('t', 'w', encoding='utf-8') as fout:
        with open('data/zh.tsv', 'r', encoding='utf-8') as fin:
            for line in fin:
                ...
                pnyn_clauses = re.sub(u"(?<=([。，！？]))", r"|", pnyn_sent).split("|")
                hanzi_clauses = re.sub(u"(?<=([。，！？]))", r"|", hanzi_sent).split("|")
                ...
                for p_clause, h_clause in zip(all_pnyn, all_hanzi):
                    ...
                    if len(p_clause) != len(h_clause):
                        print(f"Warning: length mismatch in clause: {p_clause} vs {h_clause}")
                        continue
...
def load_test_data():
    ...
    try:
        ...
        raise IOError(f"Test file {input_file} not found. Write sentences in `eval/input.csv`.")
...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x = torch.from_numpy(np.frombuffer(self.X_bytes[idx], dtype=np.int32).copy()).int()
        y = torch.from_numpy(np.frombuffer(self.Y_bytes[idx], dtype=np.int32).copy()).int()
        return x, y

def collate_batch(batch):
    x_tensors, y_tensors = zip(*batch)
    max_len = max(t.size(0) for t in x_tensors)
    padded_x = [torch.nn.functional.pad(t, (0, max_len - t.size(0)), value=0) for t in x_tensors]
    padded_y = [torch.nn.functional.pad(t, (0, max_len - t.size(0)), value=0) for t in y_tensors]
