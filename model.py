...
from modules import (
    Embedding, Conv1d, Conv1dBanks, Prenet,
    HighwayNet, GRU, Normalize
)
...
class Graph(nn.Module):
    def __init__(self, pnyn_vocab_size, hanzi_vocab_size):
        ...
        # Prenet
        self.prenet = Prenet(hp.embed_size, [hp.embed_size, hp.embed_size//2], hp.dropout_rate)
        ...
        self.conv_proj1 = Conv1d(bank_out_channels, hp.embed_size//2, 5, padding='same')
        self.norm_proj1 = Normalize(hp.embed_size//2, hp.norm_type)
        self.conv_proj2 = Conv1d(hp.embed_size//2, hp.embed_size//2, 5, padding='same')
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            ...
            mask = (y != 0).float()
            loss = F.cross_entropy(logits.view(-1, self.hanzi_vocab_size),
